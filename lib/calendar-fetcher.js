  ...
  try {
    ...
    for (const event of data.items || []) {
      ...
      if (isAllDay || durationHours >= 3) {
        ...
        // Izlaiž pagātnes notikumus — ja sākuma datums ir pirms šodienas, nerādīt
        // (Google all-day event end.date ir nākamā diena, tāpēc pārbaudām pēc start)
        if (eventStart.getTime() < today.getTime() && eventEnd.getTime() <= today.getTime()) continue;
        ...
        // Krāsas: šodien=zaļš, <7d=oranžs, >7d=pelēks
        let color;
        if (d === 0) color = '#30d158';
