  ...
  try {
    ...
    for (const space of spacesData.spaces || []) {
      ...
      if (cRes.ok) {
        ...
        console.log(`[ClickUp] Space "${space.name}" completed: ${(cData.tasks || []).length} uzdevumi`);
        for (const t of (cData.tasks || []).slice(0, 20)) {
          const creator = t.creator?.username || t.creator?.email || '';
          const desc = (t.text_content || t.description || '').split('\n').filter(l => l.trim()).slice(0, 3).join(', ');
          console.log(`[ClickUp] Completed: "${t.name}" by ${creator}, desc: ${desc.substring(0, 50)}`);
...
export function mapPerson(initials) {
  ...
  if (i.includes('EL') || i.includes('EB') || i.includes('EG')) return 'EL';
