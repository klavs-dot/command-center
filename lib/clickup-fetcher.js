...
export async function fetchClickUpData() {
  ...
  try {
    ...
    for (const space of spacesData.spaces || []) {
      ...
      if (tRes.ok) {
        for (const t of (await tRes.json()).tasks || []) {
          const creator = t.creator?.username || t.creator?.email || '';
          allActive.push({ id: t.id, name: t.name, status: t.status?.status,
            assignee: getInitials(t.assignees), creator: getCreatorInitials(creator),
      ...
      // ═══ PABEIGTI uzdevumi ═══
      // Izmantojam include_closed=true un filtrējam pēc status type
      const listsRes = await fetch(`https://api.clickup.com/api/v2/space/${space.id}/list`, { headers });
      if (listsRes.ok) {
        const lists = (await listsRes.json()).lists || [];
        for (const list of lists) {
          const cRes = await fetch(
            `https://api.clickup.com/api/v2/list/${list.id}/task?include_closed=true&order_by=updated&reverse=true&page=0&subtasks=false`,
            { headers }
          );
          if (cRes.ok) {
            const tasks = (await cRes.json()).tasks || [];
            for (const t of tasks) {
              // Ņemam tikai tos, kur status type ir "closed" vai "done"
              if (t.status?.type === 'closed' || t.status?.type === 'done') {
                const creator = t.creator?.username || t.creator?.email || '';
                const desc = (t.text_content || t.description || '').split('\n').filter(l => l.trim()).slice(0, 3).join(', ');
                allCompleted.push({
                  id: t.id, name: t.name, assignee: getInitials(t.assignees),
                  creator: getCreatorInitials(creator), description: desc,
                  space: space.name, list: list.name || '', dateClosed: t.date_closed || t.date_updated
                });
              }
            }
          }
    ...
    // Sortējam completed pēc dateClosed (jaunākie pirmie)
    allCompleted.sort((a, b) => Number(b.dateClosed || 0) - Number(a.dateClosed || 0));
    ...
    console.log(`[ClickUp] ${allActive.length} aktīvi, ${allCompleted.length} pabeigti, ${overdue.length} kavējas, ${unassigned.length} neuzņemtie`);
    return { completed: allCompleted.slice(0, 15), overdue: overdue.slice(0, 15), unassigned: unassigned.slice(0, 15) };
...
export function mapPerson(initials) {
  ...
  if (i.includes('PA') || i.includes('PL') || i.includes('PS')) return 'Paula';
  if (i.includes('EL') || i.includes('EB') || i.includes('EG')) return 'Elīza';
  if (i.includes('KL') || i.includes('KA') || i.includes('KĀ')) return 'Klāvs';
  return i || '??';
}
