...
export async function fetchAsanaData() {
  ...
  try {
    ...
    // ═══ SUBTASKI — fetchojam katram aktīvam uzdevumam ═══
    const SUBTASK_FIELDS = 'name,assignee,assignee.name,completed,due_on';
    for (const task of active) {
      const subs = await api(`/tasks/${task.id}/subtasks?opt_fields=${SUBTASK_FIELDS}&limit=50`, token);
    ...
    console.log(`[Asana] Active: ${cleanActive.length}, Completed: ${completed.length}, Upcoming by person:`, byPerson);

    return {
      completed: completed.slice(0,35),
      overdue: overdue.slice(0,15),
      unassigned: unassigned.slice(0,15),
      upcoming: upcoming.slice(0,30),
      events: events.slice(0,10),
      totalActive: cleanActive.length,
